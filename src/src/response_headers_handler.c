...
void response_headers_handler_initialize(ResponseHeadersHandler *handler)
{
	...
	handler->responseProperties.expiration = 0;
	handler->responseProperties.websiteRedirectLocation = 0;
	...
	handler->responseProperties.allowOrigin = 0;
	handler->responseProperties.allowHeaders = 0;
	handler->responseProperties.maxAge = 0;
	handler->responseProperties.allowMethods = 0;
	handler->responseProperties.exposeHeaders = 0;
    handler->done = 0;
                                  ...
                                  char *header, int len)
{
    ...
    *end = 0;
    ...
    if (!strncmp(header, "x-amz-request-id", namelen)) {
    ...
    else if (!strncmp(header, "x-amz-expiration", namelen)) {
        responseProperties->expiration= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "x-amz-website-redirect-location", namelen)) {
        responseProperties->websiteRedirectLocation= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "x-amz-version-id", namelen)) {
        responseProperties->versionId= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "Access-Control-Allow-Origin", namelen)) {
        responseProperties->allowOrigin= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "Access-Control-Allow-Headers", namelen)) {
        responseProperties->allowHeaders= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "Access-Control-Max-Age", namelen)) {
        responseProperties->maxAge= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "Access-Control-Allow-Methods", namelen)) {
        responseProperties->allowMethods= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
    else if (!strncmp(header, "Access-Control-Expose-Headers", namelen)) {
        responseProperties->exposeHeaders= 
            string_multibuffer_current(handler->responsePropertyStrings);
        string_multibuffer_add(handler->responsePropertyStrings, c, 
                               valuelen, fit);
    }
	else if (!strncmp(header, "x-amz-server-side-encryption", namelen)) {
		responseProperties->serverSideEncryption= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (!strncmp(header, "x-amz-server-side-encryption-aws-kms-key-id", namelen)) {
		responseProperties->kmsKeyId= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (!strncmp(header, "x-amz-server-side-encryption-customer-algorithm", namelen)) {
		responseProperties->customerAlgorithm= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (!strncmp(header, "x-amz-server-side-encryption-customer-key-MD5", namelen)) {
		responseProperties->customerKeyMD5= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
        ...
        S3NameValue *metaHeader = 
            &(handler->responseMetaData
              [handler->responseProperties.metaDataCount++]); //lint !e409 !e52
