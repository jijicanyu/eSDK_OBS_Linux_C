...
char* string_To_UTF8(const char* strSource)
{
	...
	*/
	...
	memset_s(pwBuf, sizeof(wchar_t) * (nwLen+1), 0,  sizeof(wchar_t) * (nwLen+1));//修改memset_s()函数的count

	MultiByteToWideChar(CP_ACP, 0, strSource, -1, pwBuf, nwLen);//jwx329074 2016.10.10 可以设置为-1，会自动判断lpMultiByteStr指定的字符串的长度
	...
	*/
	nLen = WideCharToMultiByte(CP_UTF8, 0, pwBuf, -1, NULL, 0, NULL, NULL);//先使cbMultiByte为0调用WideCharToMultiByte一次以获得所需缓冲区大小

	pBuf = (char *)malloc(sizeof(char) * (nLen + 1));//为缓冲区分配空间
	if (NULL == pBuf)
	{
		COMMLOG(OBS_LOGERROR, "Malloc pBuf failed!");
		free(pwBuf);        //zwx367245 2016.10.08 return之前先释放已申请的内存 
		pwBuf=NULL;
		return NULL;
	}
	...
	WideCharToMultiByte(CP_UTF8, 0, pwBuf, -1, pBuf, nLen, NULL, NULL);// jwx329074 2016.10.10 可以设置为-1，会自动判断lpMultiByteStr指定的字符串的长度
...
char* UTF8_To_String(const char* strSource)
{
	...
	if (NULL == str)
	{
		...
		free(wstr);        //zwx367245 2016.10.08 return之前先释放已申请的内存 
		wstr=NULL;
		return NULL;
	}
...
static void SHA1_transform(uint32_t state[5], const unsigned char buffer[64])
{
    ...
    //memcpy_s(block, sizeof(u), buffer, 64); //zwx367245 2016.10.08 休正参数从sizeof(block)修改为sizeof(u)，block只是个u*指针
