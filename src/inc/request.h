...
#ifdef WIN32
#define LIBS3_VER_MAJOR "1.5"
#define LIBS3_VER_MINOR "70"
#endif
...
//OPTIONS 对象作为请求
typedef struct S3CorsConf
{
	const char *origin;
	const char *requestMethod[100];
	unsigned int rmNumber;
	const char *requestHeader[100];
...
S3Status request_api_initialize(const char *userAgentInfo, int flags,
                                const char *hostName,S3Authorization auth ,const char* defaultRegion);
