...
// https://obs.huawei.com/${BUCKET}/${KEY}?acl
...
int getTimeZone();
// Change time format Add by cwx298983 2016.7.25 Start
...
//  Compute HMAC-SHA-256 with key [key] and message [message], storing result in [hmac]
//	The string's length is 32 bytes,and the string could be transformed to 16 Hexadecimal when it is used.
void HMAC_SHA256(unsigned char hmac[32], const unsigned char *key, int key_len,
               const unsigned char *message, int message_len);

//	COMPUTE SHA256Hash with message [message], stoing result in [sha]
//	The string's length is 32 bytes,and the string could be transformed to 16 Hexadecimal when it is used.
